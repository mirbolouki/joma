    ...
    '<div class="pet-grid">'+
      '<div class="pet-col">'+
        ...
        note('سنجهٔ چهارم («وقت حمام» — تمرین تنفس) تا ساخته‌شدن منبعش در بک‌اند نمایش داده نمی‌شود؛ '+
          ...
          'پاداش فقط از **ثبت معتبر** می‌آید — و **آبِ پیش‌نویس در سنجه‌ها شمرده نمی‌شود** — آب با «ثبت نهایی آب امروز» قطعی می‌شود (`WTR-10`).')+
      '</div>'+
    '<div class="grid2">'+
      '<div class="card"><h3>'+ic('i-heart')+'مراقبت امروز — سه عدد واقعی</h3>'+
        METERS.map(function(m){
          ...
          return '<div class="pmeter"><span class="pe">'+m.e+'</span>'+
            '<span class="pl"><b>'+m.n+'</b><small>'+m.s+
              (m.k==='water'&&!APP.waterFinal?' — <b>پیش‌نویس، نشمرده</b>':'')+'</small></span>'+
...
/* ============================ ۸) دفترچهٔ جوما — سند ۱۵ ============================
   دو بخش: «از ثبت‌های تو» (بینش واقعی با شواهد اجباری) و «نوشته‌های خودت».
   هیچ درصد اطمینان، هیچ نمودار داخل کارت، هیچ گیمیفیکیشن — سند ۱۵ §۹. */
var INSIGHTS=[
  {type:'personal', kind:'بهترین روز هفته', e:'owl-hi', tag:'از ثبت‌های تو', date:'۱۴ شهریور ۱۴۰۵',
   text:'در ثبت‌های این بازه، <b>پنجشنبه‌ها</b> بیشترین ثبت را داشته‌ای. این یک همراهی است، نه قانون — '+
        'شاید فقط برنامه‌ات آن روزها سبک‌تر است.',
   meta:'۹ هفته · ۳۴ روز ثبت‌شده', ev:true},
  {type:'personal', kind:'حال × موفقیت روزانه', e:'owl-think', tag:'از ثبت‌های تو', date:'۱۳ شهریور ۱۴۰۵',
   text:'در ثبت‌های این بازه، روزهایی که «کیفیت خواب» بالاتر بوده، درصد موفقیت روزانه‌ات هم بالاتر دیده می‌شود. '+
        '<b>همراهی، دلیل نیست.</b>',
   meta:'۵ جفت داده · بازهٔ ۱۴ تا ۲۷ شهریور', ev:true},
  {type:'personal', kind:'بلندترین رشتهٔ ثبت', e:'owl-cheer', tag:'از ثبت‌های تو', date:'۱۱ شهریور ۱۴۰۵',
   text:'بلندترین رشتهٔ ثبت پشت‌سرهم تو <b>۱۲ روز</b> بوده. این را از تاریخ ثبت‌ها می‌شماریم، نه از حس تو.',
   meta:'۱۲ روز پشت‌سرهم · تا ۱۱ شهریور', ev:true},
  {type:'personal', kind:'پایدارترین فعالیت', e:'i-walk', tag:'از ثبت‌های تو', date:'۱۰ شهریور ۱۴۰۵',
   text:'«پیاده‌روی» پایدارترین فعالیت ثبت‌شدهٔ توست؛ «مطالعه» کم‌ثبت‌ترین. هیچ‌کدام قضاوت نیست — فقط شمارش.',
   meta:'۳۱ روز · ۲ فعالیت', ev:true},
  {type:'personal', kind:'آب قطعی‌شده', e:'i-drop', tag:'از ثبت‌های تو', date:'１０ شهریور ۱۴۰۵',
   text:'۴۲٪ ثبت‌های آب تو تا امروز <b>قطعی</b> شده‌اند؛ بقیه پیش‌نویس مانده‌اند. فقط قطعی‌ها در سنجهٔ آب حساب می‌شوند.',
   meta:'۲۱ رکورد آب · ۹ قطعی', ev:true},
  {type:'feedback', kind:'بازخورد ثبت', e:'owl-cheer', tag:'ثبت امروزت', date:'۱۳ شهریور ۱۴۰۵',
   text:'امروز ۴ کار ثبت کردی و نوشیدنت کامل شد. هفتهٔ پیش در همین روز ۲ کار ثبت کرده بودی.',
   meta:'از ثبت‌های ۱۳ شهریور'},
  {type:'reminder', kind:'یادآوری', e:'owl-think', tag:'یادآوری', date:'۱۲ شهریور ۱۴۰۵',
   text:'دفترچهٔ جوما وقتی چند روز داده داشته باشد، الگوها را نشان می‌دهد.',
   meta:'راهنمای محصول'}
];
var BOOK_NOTES=[
  {d:'۱۴ شهریور ۱۴۰۵', t:'امروز بعد از پیاده‌روی حالم خیلی بهتر بود.', m:'کنار ثبت حال همین روز'},
  {d:'۱۲ شهریور ۱۴۰۵', t:'دیشب زودتر خوابیدم و صبح راحت‌تر بیدار شدم.', m:'کنار ثبت حال همین روز'},
  {d:'۹ شهریور ۱۴۰۵',  t:'حواسم پرت بود؛ ولی آب را کامل ثبت کردم.', m:'کنار ثبت حال همین روز'}
];
function R_book(){
  var f=APP.journalFilter||'all';
  var sec=APP.bookSec||'ins';
  var list=INSIGHTS.filter(function(x){return f==='all'||x.type===f;});
  if(empty()) list=[];

  var secs='<div class="filterchips">'+
    [['ins','از ثبت‌های تو'],['notes','نوشته‌های خودت']].map(function(s){
      return '<button class="'+(sec===s[0]?'btn primary sm':'btn ghost sm')+'" data-booksec="'+s[0]+'">'+s[1]+'</button>';
    }).join('')+'</div>';

  var insBody = '<div class="filterchips">'+
    ...
    (list.length?list.map(function(x,i){
      return '<article class="insight-card '+x.type+' fadeup">'+
        '<header>'+owl(x.e,40)+'<div><span class="chip '+
          ...
          '<span class="tiny" style="margin-inline-start:8px">'+x.kind+' · '+x.date+'</span></div></header>'+
      ...
      '<p class="tiny">وقتی داده کافی نباشد، <b>هیچ بینشی ساخته نمی‌شود</b> — بی‌آنکه عددی جعل شود. '+
      'برای اولین برداشت، به حدود ۱۰ روز ثبت نیاز است.</p></div>');

  var notesBody='<div class="filterchips">'+
      '<input class="inp" data-noteq placeholder="جست‌وجو در نوشته‌های خودت" style="max-width:260px" value="">'+
    '</div>'+
    (empty()||!BOOK_NOTES.length?
      '<div class="nodata">'+owl('owl-think',62,'floaty')+'<b>هنوز جمله‌ای ننوشته‌ای</b>'+
      '<p class="tiny">در گام آخر «حال من» می‌توانی یک جملهٔ اختیاری بنویسی؛ همان‌جا اینجا می‌آید.</p></div>'
      :
      '<div class="card"><h3>'+ic('i-file')+'نوشته‌های خودت</h3>'+
      BOOK_NOTES.map(function(n){
        return '<div class="st"><span><b>'+n.t+'</b><small>'+n.m+'</small></span>'+
          '<span class="end"><span class="tiny">'+n.d+'</span> '+
          '<button class="btn ghost sm" data-noteedit>ویرایش</button> '+
          '<button class="btn ghost sm" data-notedel>پاک‌کردن</button></span></div>';
      }).join('')+
      '<div class="tfoot"><span class="tiny">صفحهٔ ۱ از ۳</span><span class="sp"></span>'+
        '<button class="btn ghost sm">‹ تازه‌تر</button><button class="btn ghost sm">قدیمی‌تر ›</button></div>'+
      '<p class="tiny" style="margin-top:8px">این جمله‌ها همان <b>جملهٔ شخصی</b> ثبت حال‌اند. '+
        '<b>پاک‌کردن یادداشت، رکورد حال را دست نمی‌زند</b> — فقط متن می‌رود، عددها می‌مانند. '+
        'و هیچ‌کدام از این نوشته‌ها <b>هرگز به هم‌مسیر نمی‌رود</b> (<code>VIEW_NOTES</code>).</p></div>');
  ...
  return head('دفترچهٔ جوما','چیزهایی که جوما در ثبت‌های تو دیده','هر کارت می‌گوید از کدام ثبت آمده و چقدر داده پشتش است.',
    ...
    secs+
    (sec==='ins'?insBody:notesBody)+

    note('سه قاعدهٔ سخت: هر بینش <b>شواهد</b> دارد («از کجا آمده»)، <b>حدس را شاهد جا نمی‌زند</b> '+
      'و <b>«همراهی، دلیل نیست»</b> داخل خودِ متن است. این صفحه هیچ عددی به نام «اطمینان» ندارد، '+
      'هیچ <b>نمودار داخل کارت</b> نمی‌کشد و هیچ <b>گیمیفیکیشن</b> ندارد؛ و اگر داده کم باشد، بینش <b>ساخته نمی‌شود</b>.');
}
