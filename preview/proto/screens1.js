...
function authFoot(active){
  return '<div class="auth-promise">'+ic('i-info')+
    '<span><b>در این نمونه:</b> '+({login:'برای دیدن خانهٔ ساخته‌شده، همین‌طور «ورود» را بزن.',
        ...
        forgot:'کد بازیابی ساخته و روی تست آزمایش شد: کد یک‌بارمصرف را سرور صادر می‌کند و پشتیبانی دستی می‌دهد (۲۲ §۴).'}[active]||'نمونه فقط برای بازبینی است.')+
    '</span></div>'+
...
/* ---------- وضعیت واقعی مسیر بازیابی رمز (دور ۲۳) ----------
   مرحلهٔ ۱ (بستن موقت) و مرحلهٔ ۲ (کد یک‌بارمصرف) هر دو روی تست ساخته و آزمایش شدند.
   این بنر در همهٔ گام‌ها نشان داده می‌شود تا کاربر و بازبین بدانند امروز چه چیزی واقعی است. */
function forgotStatus(){
  return '<div class="banner info tiny" style="margin-bottom:12px">'+ic('i-checkc')+
    '<span><b>روی تست ساخته و روی تست آزمایش شد — دور ۲۳.</b> کد ۶ رقمی <b>یک‌بارمصرف</b> را '+
    '<b>سرور</b> صادر می‌کند و <b>پشتیبانی</b> دستی به تو می‌دهد (<span dir="ltr">۰۹۹۶۷۹۷۹۴۷۱</span>). '+
    'کد فقط <b>یک‌بار</b> نشان داده می‌شود · <b>۱۵ دقیقه</b> اعتبار · <b>۵ تلاش</b>؛ '+
    'و پس از تغییر رمز، <b>همهٔ نشست‌های دیگر بسته می‌شوند</b>.</span></div>';
}
...
function R_forgot(){
  ...
  var hd=forgotStatus()+'<div class="fg-hd">'+ic('i-lock')+'<b>گام '+fa(st+1)+' از '+fa(4)+'</b>'+dots+'</div>';
  ...
  if(st===1){
    return authShell('forgot','کد یک‌بارمصرف','کد را از پشتیبانی بگیر.', hd+
      ...
      note('کد ۱۵ دقیقه اعتبار دارد، یک‌بارمصرف است و ۵ بار می‌شود امتحانش کرد. '+
        'اگر عمرش تمام شود، پیام سرور همین است: «اعتبار کد تمام شده است. کد تازه بگیر.» و در هر تلاش اشتباه، '+
        'تعداد تلاش باقی‌مانده نشان داده می‌شود (مثلاً «۴ تلاش باقی مانده»). '+
        'پشتیبانی <b>هرگز</b> رمز نمی‌پرسد و رمز تازه هم نمی‌سازد — فقط کد می‌دهد و <b>بدون تأیید هویت</b> کد نمی‌دهد.'));
  }
  if(st===2){
    return authShell('forgot','رمز تازه','حالا یک رمز تازه بساز.', hd+
      ...
      note('پس از ذخیره، <b>همهٔ نشست‌های دیگر باطل می‌شوند</b> و باید از نو وارد شوی. '+
        'چون پیامک و ایمیلِ محصولی نداریم، <b>هیچ پیامی به کانال فرستاده نمی‌شود</b>؛ به‌جایش وقتی آن دستگاه‌ها '+
        'صفحه‌ای را باز کنند، می‌خوانند: «برای امنیت، نشست‌های قبلی بسته شدند».'));
  }
  return authShell('forgot','رمزت عوض شد ✓','حالا با رمز تازه وارد شو.', hd+
    '<div class="fg-done">'+timeSticker(56)+owl('owl-cheer',96,'floaty')+
      '<p class="sub">«رمز عوض شد.» از دستگاه‌های دیگر خارج شدی؛ دفعهٔ بعد که آن‌ها را باز کنی می‌خوانند: «برای امنیت، نشست‌های قبلی بسته شدند».</p></div>'+
...
function R_today(){
  ...
  return head('امروز','کارهای امروز',D('۴ از ۷ کار امروز ثبت شده.','برنامه‌ات هنوز فعالیتی ندارد.'),
    ...
    note('«ثبت نهایی» قفل روز را می‌بندد. افزودن یا حذف فعالیت روی دورهٔ در حال اجرا <b>ممنوع دائمی</b> است؛ '+
      'اگر حال امروز ثبت نشده باشد، «برنامهٔ من» و «گزارش‌ها» <b>باز می‌مانند</b> و فقط همین صفحه یک یادآوری نشان می‌دهد — بدون قفل (تصمیم دور ۲۱). '+
      'حذف '+
