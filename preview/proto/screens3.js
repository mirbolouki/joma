...
function R_settings(){
  ...
  if(tab==='profile'){
    ...
    return t+tabs+
      ...
      '<div class="card"><h3>'+ic('i-link')+'مسیرهای حساب</h3>'+
        ...
        '<div class="kv"><span>خروجی گرفتن از داده‌های من</span>'+
          '<span class="tiny">گزارش PDF/تصویر · جدول CSV · فایل پشتیبان JSON</span>'+
          '<button class="btn ghost sm" data-go="rights">باز کن</button></div>'+
  ...
  return t+tabs+
    ...
    '<div class="card"><h3>'+ic('i-lock')+'داده و حریم خصوصی</h3>'+
      ...
      '<b>خروجی و حذف داده:</b> طراحی کامل است (گزارش خوانا · CSV · JSON) و در محصول، ساختش مشروط به بسته‌شدن '+
      '<code>OPEN-12</code> است.</div>'+
    '</div>'+
    ...
    '<div class="card"><h3>'+ic('i-info')+'درباره و پشتیبانی</h3>'+
      ...
      '<div class="kv"><span>پشتیبانی — پیامک و پیام‌رسان بله</span>'+
        '<b class="num" dir="ltr">۰۹۹۶۷۹۷۹۴۷۱</b>'+
        '<button class="btn ghost sm" data-copy="09967979471">کپی</button>'+
        '<button class="btn ghost sm" data-go="content">پرسش‌های پرتکرار</button></div>'+
...
function R_content(){
  ...
  if(pg==='about'){
    ...
    var FAQ=[
      ['ثبت‌هایم پاک شده؟','نه. هر ثبت تاریخچه دارد و با برچسب «اصلاح با دلیل» نگه داشته می‌شود؛ مقدار قبلی پاک نمی‌شود.'],
      ['می‌توانم برنامه را وسط دوره عوض کنم؟','هدف‌های یک دورهٔ در حال اجرا قفل‌اند تا گزارش‌هایت معنی‌دار بماند. برای تغییر، یک دورهٔ نو بساز — داده‌های قبلی می‌ماند.'],
      ['می‌توانم ثبت فردا را انجام دهم؟','نه. فقط «امروز» و «دیروز» (با برچسب تأخیر) ثبت می‌شود؛ فردا نه.'],
      ['چرا حال دیروز را نمی‌توانم ثبت کنم؟','حال، خودگزارشیِ لحظه است؛ اگر برای روزهای گذشته ثبت شود، گزارش دیگر معنای واقعی ندارد.'],
      ['اگر چند روز نیایم، جوجه چه می‌شود؟','هیچ‌وقت نمی‌میرد و چیزی از دست نمی‌دهد. رنگش پریده می‌شود تا بگوید «دلم برایت تنگ شده» — و با اولین ثبت، برمی‌گردد.'],
      ['تمرین تنفس چطور کار می‌کند؟','تمرین ۴-۷-۸: چهار ثانیه دم، هفت ثانیه نگه‌داشتن، هشت ثانیه بازدم — با دایرهٔ انیمیشنی و شمارش. سه دور کمتر از یک دقیقه است و هیچ امتیازی نمی‌دهد.'],
      ['اگر استرسم زیاد باشد چه اتفاقی می‌افتد؟','اگر در «حال من» استرست را «زیاد» یا «خیلی زیاد» ثبت کنی، در «کارهای امروز» یک کارت پیشنهاد تنفس نشان داده می‌شود. فقط پیشنهاد است — می‌توانی با یک کلیک بگویی «بعداً».'],
      ['تفاوت کتابخانه و آموزش چیست؟','کتابخانه = فعالیت‌هایی که به برنامه‌ات اضافه می‌کنی (مثل پیاده‌روی). آموزش = راهنمای تمرین‌ها، یکی‌یکی (مثل «گوش‌دادن آینه‌ای»). هر فعالیتی که راهنمای تمرین داشته باشد، دکمهٔ «آموزش» می‌گیرد.'],
      ['چطور مشاور بگیرم؟','سه راه: درخواست از طرف مشاور و تأیید تو · کد دعوت مشاور · انتخاب مشاور از فهرست و تأیید او. تا وقتی هر دو طرف تأیید نکنند، هیچ داده‌ای رد و بدل نمی‌شود.'],
      ['مشاور من چه چیزی می‌بیند؟','فقط چیزهایی که خودت مجوز داده‌ای — به‌صورت پیش‌فرض هیچ‌چیز. هر وقت خواستی، همان‌جا خاموش می‌کنی.'],
      ['خروجی داده چه فرمت‌هایی دارد؟','سه قالب: گزارش خوانا (PDF/تصویر) برای دیدن و نشان‌دادن · جدول CSV/اکسل برای تحلیل · فایل پشتیبان کامل (JSON). اگر JSON برایت مبهم است، همان PDF را بگیر.'],
      ['حسابم را پاک کنم چه می‌شود؟','درخواست می‌دهی، ۳۰ روز مهلت پشیمانی هست و بعد دادهٔ شخصی حذف می‌شود. تا آن روز، دسترسی همراه از همان لحظه بسته است.'],
      ['رمز عبورم را فراموش کرده‌ام؟','مسیر بازیابی طراحی شده ولی تا وقتی سازوکار امنش تأیید نشده، جای دکمهٔ بازیابی با پشتیبانی تماس می‌گیری. رمز را هیچ‌وقت از تو نمی‌پرسیم.'],
      ['چرا کنترل اعلان‌ها را نمی‌بینم؟','چون زیرساخت واقعی ارسال اعلان وجود ندارد. سوییچی که روشن کنی و هیچ اتفاقی نیفتد، اعتمادت را خراب می‌کند.'],
      ['جوما چقدر هزینه دارد؟','نسخهٔ فعلی رایگان است. هر وقت پلن پولی تعریف شود، همین‌جا و در صفحهٔ شرایط نوشته می‌شود — هیچ بندی بدون محصول نوشته نمی‌شود.']
    ];
    body='<div class="content-page">'+
      '<div class="support-hero">'+
        '<div class="sh-card"><span class="sh-ic">'+ic('i-chat')+'</span>'+
          '<b>پیامک (SMS)</b><span class="sh-num num" dir="ltr">۰۹۹۶۷۹۷۹۴۷۱</span>'+
          '<div class="sh-act"><a class="btn primary sm" href="sms:09967979471">'+ic('i-chat')+'فرستادن پیامک</a>'+
          '<button class="btn ghost sm" data-copy="09967979471">کپی شماره</button></div>'+
          '<small class="tiny">برای وقتی اینترنت ضعیف است یا نمی‌خواهی وارد برنامه شوی.</small></div>'+
        '<div class="sh-card"><span class="sh-ic ind">'+ic('i-help')+'</span>'+
          '<b>پیام‌رسان بله</b><span class="sh-num num" dir="ltr">۰۹۹۶۷۹۷۹۴۷۱</span>'+
          '<div class="sh-act"><a class="btn primary sm" href="https://ble.ir" target="_blank" rel="noopener">'+ic('i-link')+'گفت‌وگو در بله</a>'+
          '<button class="btn ghost sm" data-copy="09967979471">کپی شماره</button></div>'+
          '<small class="tiny">همان شماره در پیام‌رسان بله هم هست؛ نصب از سایت رسمی ble.ir.</small></div>'+
      '</div>'+
      '<div class="card"><h3>'+ic('i-clock')+'پاسخ‌دهی</h3>'+
        '<div class="kv"><span>ساعت پاسخ</span><b>۹ تا ۲۱ — همهٔ روزها</b></div>'+
        '<div class="kv"><span>زمان پاسخ معمول</span><b>کمتر از یک روز کاری</b></div>'+
        '<div class="kv"><span>چه چیزی بفرستی تا سریع حل شود</span><b>نام کاربری + یک عکس از صفحه + کد خطا (اگر دیده‌ای)</b></div>'+
        '<div class="banner warn" style="margin-top:10px">'+ic('i-lock')+
        '<b>پشتیبانی هرگز نمی‌پرسد:</b> رمز عبور · کد یک‌بارمصرف · شمارهٔ کارت بانکی. اگر کسی این‌ها را پرسید، جوما نیست — خبر بده.</div>'+
      '</div>'+
      ...
      '<div class="card"><div class="ch-head"><h3>'+ic('i-help')+'پرسش‌های پرتکرار</h3>'+
        '<span class="chip s">'+fa(FAQ.length)+' پرسش</span></div>'+
        '<div class="faq">'+FAQ.map(function(q,i){
          var key='f'+i, open=(APP.faqOpen===key);
          return '<div class="fq'+(open?' on':'')+'">'+
            '<button class="fq-q" data-faq="'+key+'">'+q[0]+
              '<span class="fq-i">'+ic('i-chev-l')+'</span></button>'+
            (open? '<div class="fq-a tiny">'+q[1]+'</div>':'')+'</div>';}).join('')+'</div>'+
        '<div class="tiny" style="margin-top:10px">پرسشت اینجا نبود؟ با پیامک یا بله بپرس — همان شمارهٔ بالا.</div>'+
      '</div>'+
      '<div class="card not-box"><h3>'+ic('i-lock')+'چه چیزی را نمی‌توانیم برگردانیم</h3>'+
        '<p class="tiny">رمز را نمی‌دانیم، پس نمی‌توانیم بگوییم. ثبت‌های دیروزِ قطعی‌شده هم عوض نمی‌شوند.</p></div>'+
    '</div>';
  } else if(pg==='privacy'){
...
function R_rights(){
  ...
  var body='';
  if(step==='export'){
    var fmt=APP.exportFmt||'report';
    var FMTS=[
      ['report','گزارش خوانا (PDF یا تصویر)','برای دیدن خودت، یا نشان‌دادن به روانشناس و مشاورت: نمودارها، خلاصهٔ دوره و جدول‌ها — فارسی و راست‌به‌چپ.','پیشنهاد جوما'],
      ['csv','صفحه‌گسترده (CSV / اکسل)','برای وقتی می‌خواهی خودت در اکسل تحلیل کنی یا به ابزار دیگری بدهی. هر ثبت، یک سطر.','برای تحلیل'],
      ['json','فایل پشتیبان کامل (JSON)','همهٔ دادهٔ خام و ماشین‌خوان: برای انتقال بین دستگاه/حساب یا نگهداری فنی. برای کاربر عادی لازم نیست.','فنی']
    ];
    var names={report:'joma-report-1405-06-27.pdf',csv:'joma-record-1405-06-27.csv',json:'joma-data-1405-06-27.json'};
    body='<div class="card"><h3>'+ic('i-download')+'دادهٔ من</h3>'+
      '<p class="tiny">همهٔ چیزی که در جوما دربارهٔ تو ثبت شده، مال خودت است. اول بگو <b>برای چه</b> می‌خواهی، تا درست‌ترین قالب را بگیری.</p>'+
      '<div class="istep"><span class="inum">۱</span><div><b>خروجی را برای چه می‌خواهی؟</b>'+
        '<div class="fmt-list">'+FMTS.map(function(f){
          return '<button class="fmt'+(fmt===f[0]?' on':'')+'" data-exportfmt="'+f[0]+'">'+
            '<span class="fmt-radio"></span><span class="fmt-b"><b>'+f[1]+'</b><em class="tiny">'+f[2]+'</em></span>'+
            '<span class="chip'+(f[0]==='report'?' g':'')+'">'+f[3]+'</span></button>';}).join('')+'</div></div></div>'+
      ...
      '<div class="istep"><span class="inum">۳</span><div><b>دانلود</b>'+
        '<div class="dl-card"><div>'+ic('i-file')+'<b class="num">'+names[fmt]+'</b>'+
          '<div class="tiny">لینک امن، یک‌بارمصرف · انقضا ۷ روز</div></div>'+
          '<button class="btn primary sm" data-download>دانلود</button></div></div></div>'+
      '<div class="card json-explain"><h3>'+ic('i-help')+'JSON به کار من می‌آید یا نه؟</h3>'+
        '<div class="tbl-wrap"><table class="tbl"><thead><tr><th>می‌خواهی چه کار کنی؟</th><th>قالب درست</th></tr></thead><tbody>'+
        [['پیشرفتم را ببینم یا به روانشناس نشان بدهم','گزارش خوانا (PDF/تصویر)'],
         ['خودم در اکسل تحلیل کنم یا به ابزار دیگری بدهم','صفحه‌گسترده (CSV)'],
         ['همهٔ داده را دقیق نگه دارم، یا به دستگاه/حساب دیگری منتقل کنم','فایل پشتیبان کامل (JSON)'],
         ['نمی‌دانم JSON چیست','لازم نیست بدانی — همان PDF را بگیر']]
        .map(function(r){return '<tr><td class="tiny">'+r[0]+'</td><td><b class="tiny">'+r[1]+'</b></td></tr>';}).join('')+
        '</tbody></table></div>'+
        note('JSON یک فایل متنی ماشین‌خوان است — برای بکاپ و انتقال ساخته شده، نه برای خواندن با چشم. '+
          'به همین دلیل در محصول، **PDF پیش‌فرض** است و JSON فقط با توضیح صریح انتخاب می‌شود.')+
      '</div>'+
      ...
      '<div class="banner info">'+ic('i-info')+'دادهٔ همراهت داخل این فایل نیست — او هم مالک دادهٔ خودش است. '+
      'اگر فقط همراه باشی، خروجی شامل پروندهٔ مراجعانت نیست.</div>'+
    '</div>';
  } else if(step==='delete'){
